...
- **구역별 명단** — 구역마다 이름이 쭉 나열되고, 최근 심방일과 경과일이 한눈에 보입니다.
- **심방 기록** — 심방 날짜, 구분(가정심방·병원심방·전화심방 등), 심방 내용
- **성도 등록** — 이름, 직분, 구역(펼침 메뉴에서 고르거나 새로 입력), 연락처, 기도 제목 메모
- **심방이 오래된 분이 위로** — 각 구역 안에서 마지막 심방이 오래된 순으로 정렬
- **구간별 보기** — 검색창 옆 **심방 간격** 메뉴나 상단 칩으로 그 구간의 분만 모아 봅니다.
- **구역 관리** — 구역을 미리 추가하거나 삭제 (삭제하면 그 구역 성도는 미배정으로 이동)
- **검색 · 구역 필터**
- **화면 색** — 오른쪽 위에서 시스템 설정 / 밝게 / 어둡게 선택 (쓰시는 기기에 기억됩니다)
...
| --- | --- | --- |
| 20일 이내 | 초록 | 최근에 만난 분 |
| 20~40일 | 노랑 | 슬슬 안부를 여쭐 때 |
| 40~60일 | 주황 | 이번 주에 연락드릴 분 |
| 60일 이상 | 빨강 | 오래 못 뵌 분 |
| 기록 없음 | 회색 | 아직 심방 기록이 없는 분 |

상단 칩과 **심방 간격** 메뉴 모두 각 구간에 몇 분이 있는지 숫자를 보여 주며, 어느 쪽으로 골라도 같이 움직입니다.
...
4. 구역을 미리 만들어 두시려면 **구역 관리** 를 누르세요. 성도 추가 창의 구역 메뉴에서도 열 수 있습니다.
...
교구장·구역장이 한 명단을 같이 본다면 `config.js` 의 `spaceId` 에 같은 이름(예: `"1교구"`)을 적고,
